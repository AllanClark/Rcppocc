...
#pragma omp parallel for
  for (int i=0; i<n; i++){
    //Rcpp::Rcout << "i=" << i << "'A cont' = \n" << D(i)*S.slice(i)  << std::endl;
    //Rcpp::Rcout << "i=" << i << "'D(i)' = \n" << D(i)  << std::endl;
...
arma::mat matrix_multiplication(arma::mat x, arma::mat y, int ncores){
   ...
   * x %*% y and is faster than armidillo's x*y
   */
  //omp_set_num_threads(ncores);
...
arma::mat quadform3(arma::mat X, arma::vec dmat, int ncores) {
  ...
  //return matrix_multiplication(X.t(), X, ncores);

  omp_set_num_threads(ncores);

  const int p=X.n_cols, n=p;
  arma::mat C(p,p);
...
arma::mat xtx(arma::mat x, int ncores){
  //arma::mat xtx(arma::mat x, const int ncores){
  //Here we calculate x'x where y=x
...
arma::mat quadform4(arma::mat X, arma::vec dmat, int ncores) {
  //arma::mat quadform4(arma::mat X, arma::vec dmat, const int ncores) {
  //Calculation of a quadratic form X'*D*X where D is diagonal
...
arma::mat rmatmult2(arma::mat X, arma::vec Dv, int nsamps, int ncores) {
  /*Calculation of a quadratic form X'*D*X where D is diagonal
   * using random matrix sampling
   */

  omp_set_num_threads(ncores);
  int n = X.n_rows;
  int p = X.n_cols;
  arma::vec probs(n); //initialise
  arma::vec colused(1);//initialise
  arma::vec colindex = seq_int(1, n); //initialise
  ...
  for (int i=0; i < n; i++){
    probs(i) = pow(norm( X.row(i), 2 ), 2);
  }
  probs = probs/sum(probs); //norm(X, 2);
  //Rcpp::Rcout << "probs = \n" << probs.t() << std::endl;

  arma::mat ApproxXtX(p, p);
  ApproxXtX.fill(0);

  //#pragma omp parallel for
  for (int isamp=0; isamp<nsamps; isamp++){
    colused = Rcpp::RcppArmadillo::sample(colindex, 1, FALSE, probs);
    ApproxXtX += X.row(colused(0)-1).t()*X.row(colused(0)-1)/probs(colused(0)-1);
  }

  return ApproxXtX/nsamps;
}
...
// [[Rcpp::depends("RcppArmadillo")]]
// [[Rcpp::export]]
List logitoccSPAT2(arma::mat X, arma::mat W_vb, arma::mat Y, arma::mat z, arma::vec ysum, arma::vec nvisits,
                   arma::mat K, arma::mat Minv, //arma::mat Mt,
                   double n_obs,
                   NumericVector siteids, arma::vec unsurveyed_ind,
                   double tau_0, double a_tau, double b_tau,
                   arma::mat alpha_m, arma::mat beta_m,
                   arma::mat sigma_inv_alpha_p, arma::mat sigma_inv_beta_p,
                   int ndraws, double percent_burn_in, int ncores){
  ...
  for (int isamples=0; isamples<ndraws; isamples++){
    ...
    //td = timer.toc();
    //Rcpp::Rcout << "'if statement' timer = " << td << std::endl;

    //Generate the Poly-Gamma variables for beta vector
    //pg_beta = as<arma::vec>( parallelMatrixRpg(wrap(Xs_beta + Ks_theta)) );
    /*timer.tic();
     pg_beta = rpg5(Xs_beta + Ks_theta);
     Rcpp::Rcout << "'pg_beta' timer = " << timer.toc() << std::endl;
     */

    //timer.tic();
    pg_beta = rpg5_openmp(Xs_beta + Ks_theta, 3);
    //td = timer.toc();
    //Rcpp::Rcout << "'pg_beta' New timer = " << td << std::endl;
    //-----------------------------------------------------------------------
    ...
    //not much difference. will only show significant different with large X matrices!
    //beta = mvrnormArma4(1, Xs_t*( z.rows(0,n_obs-1) - 0.5 - Ks_theta % pg_beta ) + const2 , sigma_inv_beta_p  + quadform2(Xs, pg_beta));
    //td = timer.toc();
    //Rcpp::Rcout << "Beta new timer = " << td << std::endl;
    ...
    //Xs_beta = Xs*beta; //arma::mat
    //  td = timer.toc();
    //Rcpp::Rcout << "'Xs_beta' timer = " << td << std::endl;
    //-----------------------------------------------------------------------
    ...
    //timer.tic();
    //matt1 = Xs_beta % pg_beta;
    //td = timer.toc();
    //Rcpp::Rcout << "matt1 timer (Xs_beta % pg_beta) = " << td << std::endl;

    //timer.tic();
    //matt2 = z.rows(0, const1);
    //td = timer.toc();
    //Rcpp::Rcout << "matt2 timer (z.rows(0, const1)) = " << td << std::endl;

    //timer.tic();
    //matt3 = Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  );
    //td = timer.toc();
    //Rcpp::Rcout << "matt3 timer (Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  )) = " << td << std::endl;

    /*Rcpp::Rcout << "nrow(Ks) = " << Ks.n_rows << std::endl;
     Rcpp::Rcout << "ncol(Ks) = " << Ks.n_cols << std::endl;
     Rcpp::Rcout << "nrow(pg_beta) = " << pg_beta.n_rows << std::endl;
     Rcpp::Rcout << "ncol(pg_beta) = " << pg_beta.n_cols << std::endl;
     */

    /*timer.tic();
     matt4 = tau*Minv + quadform2(Ks, pg_beta);
     td = timer.toc();
     Rcpp::Rcout << "matt4 timer (tau*Minv + quadform2(Ks, pg_beta))) = " << td << std::endl;
     */

    /*timer.tic();
     matt5 = tau*Minv + quadform3(Ks, pg_beta, 20);
     td = timer.toc();
     Rcpp::Rcout << "matt5 timer (tau*Minv + quadform3(Ks, pg_beta, 10))) = " << td << std::endl;
     */

    /*
     timer.tic();
     matt6 = tau*Minv + quadform4(Ks, pg_beta);
     td = timer.toc();
     Rcpp::Rcout << "matt6 timer (tau*Minv + quadform4(Ks, pg_beta, 10)) = " << td << std::endl;
     */
    ...
    /*
     ...
     matt7 = Ks_t*Ks;
     td = timer.toc();
     Rcpp::Rcout << "matt7 timer (Ks_t*Ks) = " << td << std::endl;
     */
    ...
    /*
     ...
     matt3 = Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  );
     matt6 = tau*Minv + quadform2(Ks, pg_beta);
     theta = mvrnormArma4(1, matt3, matt6);
     td = timer.toc();
     Rcpp::Rcout << "Theta timer 2 = " << td << std::endl;

     timer.tic();
     //theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ) , tau*Minv + quadform2(Ks, pg_beta) );
     matt3 = Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  );
     matt6 = tau*Minv + quadform3(Ks, pg_beta, 4);
     theta = mvrnormArma4(1, matt3, matt6);
     td = timer.toc();
     Rcpp::Rcout << "Theta timer 3 = " << td << std::endl;

     timer.tic();
     //theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ) , tau*Minv + quadform2(Ks, pg_beta) );
     matt3 = Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  );
     matt6 = tau*Minv + quadform4(Ks, pg_beta, 4);
     theta = mvrnormArma4(1, matt3, matt6);
     td = timer.toc();
     Rcpp::Rcout << "Theta timer 4 = " << td << std::endl;
     */

    //timer.tic();
    //theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ), tau*Minv + quadform2(Ks, pg_beta));
    theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - (Xs*beta) % pg_beta  ), tau*Minv + quadform4(Ks, pg_beta, ncores));
    //td = timer.toc();
    //Rcpp::Rcout << "Theta timer 2 - V2 = " << td << std::endl;

    /*
     timer.tic();
     //theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ) , tau*Minv + quadform2(Ks, pg_beta) );
     theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ), tau*Minv + quadform3(Ks, pg_beta, 4));
     td = timer.toc();
     Rcpp::Rcout << "Theta timer 3 - V2 = " << td << std::endl;
     */

    /*
     timer.tic();
     //theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ) , tau*Minv + quadform2(Ks, pg_beta) );
     theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ), tau*Minv + quadform4(Ks, pg_beta, 4));
     td = timer.toc();
     Rcpp::Rcout << "Theta timer 4 - V2 = " << td << std::endl;
     */

    /*timer.tic();
     randvec = rmatmult2(Ks, pg_beta, 500, 3);
     theta = mvrnormArma4(1, Ks_t*( z.rows(0, const1) - 0.5 - Xs_beta % pg_beta  ) , tau*Minv + randvec );
    ...
    Ks_theta = Ks*theta; //arma::mat
    //td = timer.toc();
    //Rcpp::Rcout << "Ks_theta = " << td << std::endl;
    //-----------------------------------------------------------------------
    ...
    tau_mat(0,0) = tau;
    //td = timer.toc();
    //Rcpp::Rcout << "'tau' timer = " << td << std::endl;
    ...
    Y_iter = Y.rows( find(convert_z_vec>0) );
    //td = timer.toc();
    //Rcpp::Rcout << "'subset of z, W and Y' timer = " << td << std::endl;
    //-----------------------------------------------------------------------
    ...
    W_alpha = W_iter*alpha; // arma::mat
    //td = timer.toc();
    //Rcpp::Rcout << "W_alpha timer = " << td << std::endl;

    /*timer.tic();
     //pg_alpha = as<arma::vec>( parallelMatrixRpg(wrap(W_alpha)) ); //arma::vec
     pg_alpha = rpg5(W_alpha); //arma::vec
     Rcpp::Rcout << "'pg_alpha' timer = " << timer.toc() << std::endl;
     */

    //timer.tic();
    pg_alpha = rpg5_openmp(W_alpha, 4);
    //  td = timer.toc();
    //Rcpp::Rcout << "'pg_alpha' New timer = " << td << std::endl;
    ...
    //td = timer.toc();
    //Rcpp::Rcout << " Alpha old timer = " << td << std::endl;

    //timer.tic();
    //twice as slow
    //alpha = mvrnormArma4(1, W_iter.t()*( Y_iter - 0.5 ) + const3 , sigma_inv_alpha_p + quadform4(W_iter, pg_alpha, ncores) );
    //td = timer.toc();
    //Rcpp::Rcout << " Alpha new timer = " << td << std::endl;
    //Rcpp::Rcout << " \n" << std::endl;
    ...
    p = 1/(1 + exp(-W_vb*alpha)); //plogis(W_vb*alpha)
    //  td = timer.toc();
    //Rcpp::Rcout << "'p' timer = " << td << std::endl;
    ...
    psi = 1/(1 + exp(-X*beta - K*theta ));
    //  td = timer.toc();
    //Rcpp::Rcout << "'psi' timer = " << td << std::endl;
    ...
    for (int idown=0; idown< n_not_observed_surveyed_sites; idown++){
      ...
      zdraw = rbinom(1,1, prob(0));
      z(not_observed_index(idown)) = zdraw(0);
      //z(not_observed_index(idown)) = rbinom3(prob(0));
    }
    //td = timer.toc();
    //Rcpp::Rcout << "'update z for surveyed sites' timer = " << td << std::endl;
    ...
    for (int idown=0; idown< n_unsurveyed; idown++){
      ...
      //z(n_obs + idown) = rbinom3(prob(0));
    } //finish sampling the z matrix
    //  td = timer.toc();
    //Rcpp::Rcout << "'update z for unsurveyed sites' timer = " << td << std::endl;
    ...
    //  td = timer.toc();
    //Rcpp::Rcout << "'Store results' timer = " << td << std::endl;

    //td = timerloop.toc();
    //Rcpp::Rcout << "'Loop' timer = " << td << std::endl;
    //Rcpp::Rcout << "\n " <<  std::endl;
  ...
  return List::create(_["alpha"]=post_alpha,
                      ...
                      _["psi"]=post_psi );
  //  td = timerloop.toc();
  //Rcpp::Rcout << "'Set up the list' timer = " << td << std::endl;
